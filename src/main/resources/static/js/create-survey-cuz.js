// <![CDATA[
	$(document).ready(function(){
		...
		$("#partten2").click(function(){
			$(this).fadeOut(300);
			$("#logChooseCover").slideUp(200);
		});

		$(".btn-get-cover").click(function(){
			$("#partten2").fadeIn(200);
			$("#logChooseCover").slideDown(300);
		});

		$(".color").click(function(){
			$(".wrap-color").removeClass("color-active");
			...
			$("#maincolor").val(""+color);
			$("#cover").val(""+color);
			$("header .cover").css("background","linear-gradient(rgba(0,0,0,0), rgba(0,0,0,0.3)), "+color+" center center no-repeat");
			
			var rgba = hex2rgb(color);
			$("body").css("background",""+rgba);
			
			var rule_1 = ".active-survey{border-left: 30px solid " +color+";}";
			var rule_2 = ".describe-survey:focus{border-bottom: 1.5px solid "+ color+";}";
			var rule_3 = "input[type='text']:focus{border-bottom: 1.5px solid "+ color+";}";
			var rule_4 = ".swap-index{background-color:"+ color +"}";
			$("style").html("");
			$("style").append(rule_1);
			$("style").append(rule_2);
			$("style").append(rule_3);
			...
			inputs.each(function(index, input){
				...
				if (!parent.hasClass("text-box")){
					...
					if (checkNull(value)){
						...
						$(input).nextAll(".mess-err").first().text("Your data have special character.");
						valReturn = false;
					}*/
				}
			});
